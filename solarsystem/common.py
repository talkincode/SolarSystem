...
def days_to_hms(days):
    total_seconds = int(days * 24 * 3600)  # 将天数转换为总秒数
    hours = total_seconds // 3600  # 计算小时数
    remaining_seconds = total_seconds % 3600  # 计算剩余的秒数
    minutes = remaining_seconds // 60  # 计算分钟数
    seconds = remaining_seconds % 60  # 剩余的秒数就是秒数
...
def days_to_dhms(days):
    # 计算整天数和剩余的部分天数
    full_days = int(days)
    remaining_days = days - full_days
    
    # 剩余部分天数转换为秒
    total_seconds = int(remaining_days * 24 * 3600)
    ...
    # 格式化为 d hh:mm:ss
    return f"{full_days} days {hours:02}:{minutes:02}:{seconds:02}"
